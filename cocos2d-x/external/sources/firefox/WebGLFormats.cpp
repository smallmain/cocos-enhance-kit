...
#define FOO(x) EffectiveFormat::x, #x, LOCAL_GL_ ## x
    ...
    // GLES 3.0.4, p147, table 3.19
    // GLES 3.0.4  p286+  $C.1 "ETC Compressed Texture Image Formats"
//cjh    AddFormatInfo(FOO(COMPRESSED_RGB8_ETC2                     ), 0, 1,1,1,0, 0,0, UnsizedFormat::RGB , false, ComponentType::NormUInt);
...
//    AddFormatInfo(FOO(COMPRESSED_SRGB8_PUNCHTHROUGH_ALPHA1_ETC2), 0, 1,1,1,1, 0,0, UnsizedFormat::RGBA, true , ComponentType::NormUInt);
    ...
    // AMD_compressed_ATC_texture
//cjh    AddFormatInfo(FOO(ATC_RGB_AMD                    ), 0, 1,1,1,0, 0,0, UnsizedFormat::RGB , false, ComponentType::NormUInt);
...
//    AddFormatInfo(FOO(ATC_RGBA_INTERPOLATED_ALPHA_AMD), 0, 1,1,1,1, 0,0, UnsizedFormat::RGBA, false, ComponentType::NormUInt);
    ...
    // KHR_texture_compression_astc_ldr
//cjh    AddFormatInfo(FOO(COMPRESSED_RGBA_ASTC_4x4_KHR          ), 0, 1,1,1,1, 0,0, UnsizedFormat::RGBA, false, ComponentType::NormUInt);
...
//cjh StaticMutex gFormatMapMutex;

const FormatInfo*
GetFormat(EffectiveFormat format)
{
//cjh    StaticMutexAutoLock lock(gFormatMapMutex);
...
//cjh     gfxCriticalError() << "Bad `packing`: " << gfx::hexa(packing.format) << ", "
...
{
//cjh    MOZ_ASSERT(!this->maxSamplesKnown);
...
//cjh    if (gfxPrefs::WebGL2CompatMode()) {
...
//cjh        DebugOnly<bool> didInsert = res.second;
